        ...
        //get test cases screenshots names (on failure only)
          if (testCasesResults[j] != 'Failed' && report.screenshotsOnlyOnFailure) {
            ...
            if (report.modifiedSuiteName) {
              ...
              screenshotsNamesOnFailure.push(report.browser + '-' + testCases[j].attr.specid + '.png');
            }
          }
      }

      //store suite data
      suitesData.push({'keyword': 'Test Case', 'name': suite.name, 'testcases': testCasesNames, 'testcasesresults':testCasesResults, 'testcasestimes': testCasesTimes, 'testcasesmessages': testCasesMessages, 'screenshotsNames':screenshotsNamesOnFailure, 'tests': suite.tests, 'failed': suite.failed, 'errors': suite.errors, 'skipped': suite.skipped, 'passed': suite.passed});
